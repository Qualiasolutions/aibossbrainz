...
const OUTPUT_DIR = '/home/qualia/Desktop/Projects/aiagents/aibossbrainz/docs/demo/gifs';
...
async function captureDropdownGif(): Promise<void> {
	...
	try {
		...
		if (currentUrl.includes('/login')) {
			...
			await page.click('button:has-text("Continue as Guest"), a:has-text("Continue as Guest")', { timeout: 5000 }).catch(() => {
		...
		for (let i = 0; i < menuItems.length; i++) {
			...
			await page.screenshot({ path: join(OUTPUT_DIR, `dropdown-item-${i}.png`) });
		}
		...
		console.log(`- dropdown-item-0.png to dropdown-item-${menuItems.length - 1}.png`);
		console.log('\nTo create a GIF, use ffmpeg like:');
		console.log(`ffmpeg -framerate 2 -i ${OUTPUT_DIR}/dropdown-item-%d.png -vf "scale=600:-1" ${GIF_PATH}`);
