-- SECURITY FIX: Add admin authorization checks to all admin RPC functions
-- Previously these SECURITY DEFINER functions had no internal auth checks,
-- allowing any authenticated user to call them directly via browser console.

-- 1. get_admin_users_with_stats - now checks caller is admin
...
-- 2. get_admin_chats_with_stats - now checks caller is admin
...
-- 3. get_recent_conversations - now checks caller is admin
...
-- 4. get_admin_user_by_id - now checks caller is admin
