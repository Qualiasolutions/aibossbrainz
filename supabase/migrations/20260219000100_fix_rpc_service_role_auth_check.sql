-- FIX: Use auth.role() instead of manually parsing JWT claims
-- The previous check parsed request.jwt.claims directly, but PostgREST
-- may set request.jwt.claim.role instead. auth.role() handles both cases.

-- 1. get_admin_users_with_stats
