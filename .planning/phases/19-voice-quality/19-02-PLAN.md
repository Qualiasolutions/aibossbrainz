...
<task type="auto">
  ...
  **Changes to `generateAudioForSegment` in both files:**
  ...
  The function signature needs to change to accept and return request IDs for sequential chaining. Modify the existing function so that:
  ...
  5. **Capturing request-id from resilience wrapper:** The `withElevenLabsResilience()` wrapper returns a `Response` object (it returns `res` from inside the callback). Capture the `request-id` header from this Response object BEFORE calling `.arrayBuffer()`. Specifically:
     ```
     const response = await withElevenLabsResilience(async () => {
       // ... fetch with previous_request_ids in body ...
       return res;
     });
     ...
     ```
     The key point is that `response.headers.get('request-id')` must be called on the Response returned by `withElevenLabsResilience()`, not inside the resilience callback. The callback returns the raw `res` from fetch, and the resilience wrapper passes it through.
  6. Change return type from `Promise<ArrayBuffer>` to `Promise<{ buffer: ArrayBuffer; requestId: string | null }>`
  ...
  **Important:** The single-voice path in both routes stays unchanged (no request stitching needed for single segments). Only the collaborative multi-segment path changes.
  ...
  <verify>
  Run `pnpm lint` to confirm no errors. Grep both files for `previous_request_ids` to confirm request stitching is present. Grep both files for `Promise.all` in the collaborative path to confirm it's been replaced with sequential generation. Grep both files for `/stream` in ElevenLabs URLs to confirm streaming endpoint is used for collaborative segments. Grep both files for `response.headers.get` to confirm request-id capture is present.
  </verify>
  <done>
  Both voice routes generate collaborative segments sequentially with `previous_request_ids` for prosody-aligned audio. Both use the streaming endpoint (`/stream`) for collaborative segments. Request-id is captured from the Response returned by withElevenLabsResilience() before calling .arrayBuffer(). No `Promise.all` remains in collaborative generation paths. Single-voice paths are unchanged.
  </done>
</task>
...
5. Both voice route files capture `request-id` via `response.headers.get('request-id')` outside the resilience wrapper callback
6. `hooks/use-greeting-speech.ts` has no `setTimeout` for auto-play trigger
7. `hooks/use-greeting-speech.ts` has `addEventListener('click', ...)` for user gesture gate
8. All collaborative segment generation is sequential (for...of loop, not Promise.all)
</verification>
...
- Request-id captured from withElevenLabsResilience() Response before .arrayBuffer() extraction
