...
**Verification math (based on actual codebase counts as of planning):**
- Total `console.*` calls before migration: ~168 across server + client .ts files
- Existing structured `logger.*` calls: ~72
...
- Server-side calls to migrate: ~138-148
- Plan 01 migrated: ~38 calls (webhook 33 + chat route 5)
- Plan 02 Task 1 migrated: ~70 calls (24 lib/server files)
- Plan 02 Task 2 migrated: ~30-40 calls (23 route handler files)
- Remaining client console.*: ~20-30
- Total structured logger calls after: existing 72 + migrated ~138 = ~210
- Percentage: 210 / (210 + 30) = ~87% → exceeds 80% target

NOTE: File counts per task (24 and 23 files) are high but each file requires only a mechanical find-and-replace of 1-4 console.* calls. The bulk nature is inherent to a logging migration sweep.
