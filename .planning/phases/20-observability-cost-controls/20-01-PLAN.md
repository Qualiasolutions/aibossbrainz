...
<task type="auto">
  ...
  <action>
1. **Create Supabase migration** for AICostLog table via MCP `apply_migration` (project ID: `esymbjpzjjkffpfqukxw`):
...
Import `logger` from `@/lib/logger` and `createServiceClient` from `@/lib/supabase/server`. Mark file with `import "server-only"`.
  ...
  <verify>
Run `grep -c "recordAICost\|getDailyAICostTotal\|getMonthlyCostSummary" lib/cost/tracker.ts` -- should return 3+ (exports + usages). Verify the migration was applied via MCP.
  </verify>
  <done>AICostLog table exists in Supabase with RLS enabled. Cost tracker module exports three functions for recording and querying cost data.</done>
</task>
...
Also migrate the remaining `console.warn` calls in this file to `logger.warn()`:
...
- Any other remaining `console.*` calls in this file
