...
<task type="auto">
  ...
  <action>
    ...
    **Step 2: Create API endpoint app/(chat)/api/chat/messages/route.ts**
    ...
    This route is authenticated (behind middleware). Routes under `app/(chat)/api/` are already auth-gated by the middleware -- no middleware.ts changes needed.
    ...
    **Step 4: Add scroll-to-load in components/chat.tsx**
    ...
    Add a `loadOlderMessages` callback. IMPORTANT: `createdAt` is stored at `message.metadata.createdAt` (not top-level) because `convertToUIMessages` maps it into `MessageMetadata`. Access it via `metadata.createdAt`:
    ```typescript
    const loadOlderMessages = useCallback(async () => {
      ...
      try {
        ...
        // createdAt lives in metadata (set by convertToUIMessages from DB createdAt)
        const oldestCreatedAt = (oldestMessage as any)?.metadata?.createdAt;
    ...
    IMPORTANT: The `messages` array from `useChat` stores `createdAt` inside `metadata.createdAt` (via `convertToUIMessages` which maps DB `createdAt` to `MessageMetadata.createdAt`). Always access via `message.metadata.createdAt`, never `message.createdAt` directly.
  </action>
  <verify>
